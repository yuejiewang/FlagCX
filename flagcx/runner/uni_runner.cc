...
FLAGCX_PARAM(UniRunnerUseDevSlicedAR, "UNIRUNNER_USE_DEV_SLICEDAR", 0);
...
flagcxResult_t uniRunnerAllReduce(const void *sendbuff, void *recvbuff,
  ...
  if (flagcxParamUniRunnerUseLocRed()) {
    ...
    FLAGCXCHECKGOTO(initUniRunnerStateRingAG(runnerState, sendbuff, recvbuff,
                                             count, datatype, op, comm),
                    res, out);
  } else if (flagcxParamUniRunnerUseDevSlicedAR()) {
    /* initialize uniRunnerState for sliced AllReduce with device api p2p */
    FLAGCXCHECKGOTO(initUniRunnerStateDevSlicedAR(
