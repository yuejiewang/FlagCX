...
flagcxResult_t flagcxC2cPlanner::findStrategy() {
  ...
  if ((commOp_ == flagcxCommOpReduceScatter ||
    ...
    startoffset = clusterOffset_ * totalCount_ / comm_->nranks;
             ...
             algorithm_ == flagcxAlgoSliced) {
    startoffset = clusterOffset_ * totalCount_ / comm_->nranks / nslices_;
  }
...
flagcxResult_t flagcxC2cPlanner::execute(const void *sendbuff, void *recvbuff,
  ...
  TRACE_CALL("refreshFunc slice 0");
  ...
  if (algorithm_ == flagcxAlgoSliced) {
    // execute refreshFunc for slice 1
    TRACE_CALL("refreshFunc slice 1");
    refreshFunc_.run(static_cast<void *>(static_cast<char *>(recvbuff) +
