...
static void dagQueueEnqueue(struct flagcxDagQueue *queue, struct flagcxDagNode *node) {
  node->next = NULL;
...
//    \     /
//     Node3
static flagcxResult_t initDagScheduler(struct flagcxDagProxyKernelState *kernelState) {
  // Initialize queues
  ...
  kernelState->dagNodes = (struct flagcxDagNode *)malloc(numNodes * sizeof(struct flagcxDagNode));
  ...
  kernelState->dagNodes[0].children = (struct flagcxDagNode **)malloc(2 * sizeof(struct flagcxDagNode *));
  ...
  kernelState->dagNodes[1].children = (struct flagcxDagNode **)malloc(1 * sizeof(struct flagcxDagNode *));
  ...
  kernelState->dagNodes[2].children = (struct flagcxDagNode **)malloc(1 * sizeof(struct flagcxDagNode *));
  ...
  INFO(FLAGCX_INIT, "DAG scheduler initialized with fixed 4-node topology (0->1,2 | 1,2->3)");
...
static flagcxResult_t processReadyQueue(struct flagcxProxyKernelState *kernelState) {
  while (!dagQueueIsEmpty(&kernelState->readyQueue)) {
    ...
    if (node->nodeType == flagcxDagNodeTypeP2p) {
      ...
      res = enqueue(
          (void *)kernelState->fifo->buffer,
          (uint64_t)node->red.input1,
          (uint64_t)node->red.input2,
          (uint64_t)node->red.output,
          node->red.count,
          node->red.nthreads,
          node->red.datatype,
          node->red.redOp,
...
static flagcxResult_t processInflightQueue(struct flagcxProxyKernelState *kernelState) {
...
void *flagcxDagProxyKernelService(void *args) {
  int groupCount = 0;
...
out:
  // destroy fifo
  res = comm->proxyState->kernelState.fifo->flagcxRedFifoDestroy();
