...
flagcxResult_t flagcxNetSocketInit(flagcxDebugLogger_t logFunction) {
  if (flagcxNetIfs == -1) {
    ...
    if (flagcxNetIfs == -1) {
      ...
      flagcxNetIfs = flagcxFindInterfaces(names, addrs, MAX_IF_NAME_SIZE, MAX_IFS);
      if (flagcxNetIfs <= 0) {
        ...
        for (int i=0; i<flagcxNetIfs; i++) {
          ...
          memcpy(&flagcxNetSocketDevs[i].addr, addrs+i, sizeof(union flagcxSocketAddress));
          FLAGCXCHECK(flagcxNetSocketGetPciPath(flagcxNetSocketDevs[i].devName, &flagcxNetSocketDevs[i].pciPath));
...
static flagcxResult_t flagcxNetSocketGetSpeed(char* devName, int* speed) {
  *speed = 0;
  ...
  if (*speed <= 0) {
    INFO(FLAGCX_NET, "Could not get speed from %s. Defaulting to 10 Gbps.", speedPath);
    *speed = 10000;
  }
  return flagcxSuccess;
}
...
flagcxResult_t flagcxNetSocketGetProperties(int dev, flagcxNetProperties_t* props) {
...
void* persistentSocketThread(void *args_) {
  struct flagcxNetSocketThreadResources* resource = (struct flagcxNetSocketThreadResources*)args_;
      ...
      do {
        ...
        for (int j=0; j<nSocksPerThread; j++) {
          ...
          if (r != NULL && r->used == 1 && r->offset < r->size) {
            r->result = flagcxSocketProgress(r->op, r->sock, r->data, r->size, &r->offset);
...
flagcxResult_t flagcxNetSocketGetNsockNthread(int dev, int* ns, int* nt) {
  ...
  if (nThreads > MAX_THREADS) {
    WARN("NET/Socket : FLAGCX_SOCKET_NTHREADS is greater than the maximum allowed, setting to %d", MAX_THREADS);
  ...
  if (nThreads == -2 || nSocksPerThread == -2) {
    ...
    snprintf(vendorPath, PATH_MAX, "/sys/class/net/%s/device/vendor", flagcxNetSocketDevs[dev].devName);
  ...
  if (nSocks > MAX_SOCKETS) {
    ...
    WARN("NET/Socket : the total number of sockets is greater than the maximum allowed, setting FLAGCX_NSOCKS_PERTHREAD to %d", nSocksPerThread);
    nSocks = nSocksPerThread * nThreads;
  }
  *ns = nSocks;
  *nt = nThreads;
  if (nSocks > 0) INFO(FLAGCX_INIT, "NET/Socket: Using %d threads and %d sockets per thread", nThreads, nSocksPerThread);
  return flagcxSuccess;
}


flagcxResult_t flagcxNetSocketListen(int dev, void* opaqueHandle, void** listenComm) {
  if (dev < 0 || dev >= flagcxNetIfs) { // data transfer socket is based on specified dev
    return flagcxInternalError;
  }
  struct flagcxNetSocketHandle* handle = (struct flagcxNetSocketHandle*) opaqueHandle;
  memset(handle, 0, sizeof(struct flagcxNetSocketHandle));
  static_assert(sizeof(struct flagcxNetSocketHandle) <= FLAGCX_NET_HANDLE_MAXSIZE, "flagcxNetSocketHandle size too large");
  struct flagcxNetSocketListenComm* comm;
  FLAGCXCHECK(flagcxCalloc(&comm, 1));
  handle->magic = FLAGCX_SOCKET_MAGIC;
  FLAGCXCHECK(flagcxSocketInit(&comm->sock, &flagcxNetSocketDevs[dev].addr, handle->magic, flagcxSocketTypeNetSocket, NULL, 1));
  FLAGCXCHECK(flagcxSocketListen(&comm->sock));
  FLAGCXCHECK(flagcxSocketGetAddr(&comm->sock, &handle->connectAddr));
  FLAGCXCHECK(flagcxNetSocketGetNsockNthread(dev, &comm->nSocks, &comm->nThreads));
...
flagcxResult_t flagcxNetSocketConnect(int dev, void* opaqueHandle, void** sendComm, flagcxNetDeviceHandle_t** /*sendDevComm*/) {
  if (dev < 0 || dev >= flagcxNetIfs) { // data transfer socket is based on specified dev
    return flagcxInternalError;
  }
  ...
  *sendComm = NULL;
  ...
  if (stage->state == flagcxNetSocketCommStateConnect) goto socket_connect_check;
  if (stage->state == flagcxNetSocketCommStateSend) goto socket_send;
  ...
  for (; i<comm->nSocks+1; i++) {
    ...
    FLAGCXCHECK(flagcxSocketInit(sock, &handle->connectAddr, handle->magic, flagcxSocketTypeNetSocket, NULL, 1));
...
socket_send:
    int done = 0;
    FLAGCXCHECK(flagcxSocketProgress(FLAGCX_SOCKET_SEND, sock, &i, sizeof(uint8_t), &done));
    if (done == 0) return flagcxSuccess;
  }
  *sendComm = comm;
  return flagcxSuccess;
}
...
flagcxResult_t flagcxNetSocketAccept(void* listenComm, void** recvComm, flagcxNetDeviceHandle_t** /*recvDevComm*/) {
  struct flagcxNetSocketListenComm* lComm = (struct flagcxNetSocketListenComm*)listenComm;
  ...
  *recvComm = NULL;
  if (stage->state == flagcxNetSocketCommStateAccept) goto socket_accept_check;
  if (stage->state == flagcxNetSocketCommStateRecv) goto socket_recv;
...
socket_recv:
    int done = 0;
    FLAGCXCHECK(flagcxSocketProgress(FLAGCX_SOCKET_RECV, sock, &sendSockIdx, sizeof(uint8_t), &done));
    if (done == 0) return flagcxSuccess;
...
flagcxResult_t flagcxNetSocketGetRequest(struct flagcxNetSocketComm* comm, int op, void* data, int size, struct flagcxNetSocketRequest** req) {
...
flagcxResult_t flagcxNetSocketGetTask(struct flagcxNetSocketComm* comm, int op, void* data, int size, struct flagcxNetSocketTask** req) {
  ...
  if (queue->tasks == NULL) {
    ...
    flagcxSetThreadName(comm->helperThread[tid], "FLAGCX Sock%c%1u%2u%2u", op == FLAGCX_SOCKET_SEND ? 'S' : 'R', comm->dev, tid, comm->cudaDev);
  }
...
flagcxResult_t flagcxNetSocketTest(void* request, int* done, int* size) {
  *done = 0;
  ...
  if (r->used == 1) { /* try to send/recv size */
    int data = r->size;
    int offset = 0;
    FLAGCXCHECK(flagcxSocketProgress(r->op, r->ctrlSock, &data, sizeof(int), &offset));

    if (offset == 0) return flagcxSuccess; /* Not ready -- retry later */

    // Not sure we could ever receive less than 4 bytes, but just in case ...
    if (offset < sizeof(int)) FLAGCXCHECK(flagcxSocketWait(r->op, r->ctrlSock, &data, sizeof(int), &offset));
    ...
    if (r->comm->nSocks > 0) {
      ...
      while (chunkOffset < r->size) {
        ...
        FLAGCXCHECK(flagcxNetSocketGetTask(r->comm, r->op, (char*)(r->data)+chunkOffset, chunkSize, r->tasks+i++));
  ...
  if (r->used == 2) { // already exchanged size
    if (r->nSubs > 0) {
      ...
      for (int i=0; i<r->nSubs; i++) {
        ...
        if (sub->result != flagcxSuccess) return sub->result;
      ...
      if (r->offset < r->size) {
        FLAGCXCHECK(flagcxSocketProgress(r->op, r->ctrlSock, r->data, r->size, &r->offset));
      }
      if (r->offset == r->size) {
        if (size) *size = r->size;
        *done = 1;
...
flagcxResult_t flagcxNetSocketIsend(void* sendComm, void* data, int size, int tag, void* mhandle, void** request) {
  ...
  FLAGCXCHECK(flagcxNetSocketGetRequest(comm, FLAGCX_SOCKET_SEND, data, size, (struct flagcxNetSocketRequest**)request));
  return flagcxSuccess;
}

flagcxResult_t flagcxNetSocketIrecv(void* recvComm, int n, void** data, int* sizes, int* tags, void** mhandles, void** request) {
  struct flagcxNetSocketComm* comm = (struct flagcxNetSocketComm*)recvComm;
  if (n != 1) return flagcxInternalError;
  FLAGCXCHECK(flagcxNetSocketGetRequest(comm, FLAGCX_SOCKET_RECV, data[0], sizes[0], (struct flagcxNetSocketRequest**)request));
  return flagcxSuccess;
}

flagcxResult_t flagcxNetSocketIflush(void* recvComm, int n, void** data, int* sizes, void** mhandles, void** request) {
  // We don't support CUDA pointers, so we don't need a flush operation
  return flagcxInternalError;
}

flagcxResult_t flagcxNetSocketCloseListen(void* opaqueComm) {
  struct flagcxNetSocketListenComm* comm = (struct flagcxNetSocketListenComm*)opaqueComm;
  if (comm) {
    int ready;
    FLAGCXCHECK(flagcxSocketReady(&comm->sock, &ready));
    if (ready) FLAGCXCHECK(flagcxSocketClose(&comm->sock));
    free(comm);
  }
  return flagcxSuccess;
}
...
flagcxResult_t flagcxNetSocketClose(void* opaqueComm) {
  ...
  if (comm) {
    ...
    if (ready) FLAGCXCHECK(flagcxSocketClose(&comm->ctrlSock));
    for (int i=0; i<comm->nSocks; i++) {
      FLAGCXCHECK(flagcxSocketReady(&comm->socks[i], &ready));
      if (ready) FLAGCXCHECK(flagcxSocketClose(&comm->socks[i]));
    }
    free(comm);
  }
  return flagcxSuccess;
