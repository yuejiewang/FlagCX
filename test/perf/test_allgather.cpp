...
int main(int argc, char *argv[]) {
  ...
  for (size_t size = min_bytes; size <= max_bytes; size *= step_factor) {
    ...
    size_t count_per_rank = count / totalProcs;

    memset(hello, 0, size / totalProcs);
    ((float *)hello)[0] = proc;
    ...
    int correct = 1;
    for (size_t src_rank = 0; src_rank < (size_t)totalProcs; src_rank++) {
      for (size_t offset = 0; offset < count_per_rank; offset++) {
        size_t recv_offset = src_rank * count_per_rank + offset;
        float expected = offset == 0 ? (float)src_rank : 0.0f;
        float actual = ((float *)hello)[recv_offset];
        if (actual != expected) {
          printf("rank %d wrong output at offset %zu (src rank %zu, local "
                 "offset %zu), expected %f, got %f\n",
                 proc, recv_offset, src_rank, offset, expected, actual);
          correct = 0;
          break;
        }
      }
    ...
    if ((proc == 0 || proc == totalProcs - 1) && color == 0 && print_buffer) {
      ...
      for (int i = 0; i < totalProcs; i++) {
        printf("%f ", ((float *)hello)[i * count_per_rank]);
      }
      printf("\n");
      printf("rank %d all-gather correctness = %d\n", proc, correct);
    }
  }
